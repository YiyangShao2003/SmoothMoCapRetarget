...
class SMPL_Parser(_SMPL):
    ...
    def axis_angle_to_rotmat(self, axis_angle: torch.Tensor) -> torch.Tensor:
        """
        Axis-angle to rotation matrix.

        param:
            axis_angle (torch.Tensor): shape: (batch_size, num_joints, 3)

        return:
            rot_mats (torch.Tensor): shape: (batch_size, num_joints, 3, 3)
        """
        # Rodrigues' formula
        theta = torch.norm(axis_angle, dim=-1, keepdim=True)  # (B, J, 1)
        axis = torch.nn.functional.normalize(axis_angle, dim=-1)  # (B, J, 3)

        # Create skew-symmetric matrix
        K = torch.zeros((axis.shape[0], axis.shape[1], 3, 3), device=axis.device)
        K[:, :, 0, 1] = -axis[:, :, 2]
        K[:, :, 0, 2] = axis[:, :, 1]
        K[:, :, 1, 0] = axis[:, :, 2]
        K[:, :, 1, 2] = -axis[:, :, 0]
        ...
        I = torch.eye(3, device=axis.device).unsqueeze(0).unsqueeze(0)  # (1, 1, 3, 3)
        rot_mats = I + torch.sin(theta).unsqueeze(-1) * K + (1 - torch.cos(theta)).unsqueeze(-1) * (K @ K)  # (B, J, 3, 3)

        return rot_mats
    
    def get_global_orientations(self, pose: torch.Tensor) -> torch.Tensor:
        """
        Calculate the global rotation matrices for each joint.

        param:
            pose (torch.Tensor): shape: (batch_size, J*3)

        return:
            global_rotmats (torch.Tensor): shape: (batch_size, J, 3, 3)
        """
        batch_size = pose.shape[0]
        num_joints = len(self.joint_names)

        # reshape pose
        global_orient = pose[:, :3].unsqueeze(1)  # (B, 1, 3)
        body_pose = pose[:, 3:].view(batch_size, -1, 3)  # (B, J-1, 3)

        # axis-angle to rotation matrix
        global_rot = self.axis_angle_to_rotmat(global_orient)  # (B, 1, 3, 3)
        body_rot = self.axis_angle_to_rotmat(body_pose)  # (B, J-1, 3, 3)

        # initialize global rotation matrices
        global_rotmats = torch.zeros(batch_size, num_joints, 3, 3, device=pose.device)
        global_rotmats[:, 0] = global_rot.squeeze(1)  # 根关节

        # calculate global rotation matrices
        for j in range(1, num_joints):
            parent = self.parents[j]
            global_rotmats[:, j] = torch.matmul(global_rotmats[:, parent], body_rot[:, j-1])
